...
if (isForwardedChannel) {
    ...
    //conn.sendMessage("120363305285153025@g.us", {text: JSON.stringify(msg, null, 2) })
    ...
    global.errormes = async (conn, { e, ctx, cmnd }) => {
        ...
        conn.sendMessage(owner[0], { text: `\`Error Log :\`\n${String(e)}` }, { quoted: ctx })
    };
    ...
    if (setting.log.msg) if (command) console.log(chalk.black(chalk.bgWhite('[ PESAN ]')), chalk.black(chalk.bgGreen(` ${(prefix + command) || type} `), q ? chalk.bgYellow(` ${q} `) : "") + '\n' + chalk.blueBright('=> Dari'), chalk.green(m.sender), chalk.yellow(m.isSelf ? conn.user.lid : m.isGroup ? m.key.participant : m.from) + '\n' + chalk.magenta('=> Di'), chalk.green(m.chat, chalk.yellow(await conn.getName(m.chat))) + '\n' + chalk.black(chalk.bgBlue(` ${moment.tz("Asia/Jakarta").format("DD MMMM YYYY HH:mm:ss")} ( Waktu Indonesia Barat ) `)))
    ...
    if (isRank && users[m.sender].limitBug == undefined) {
        users[m.sender].limitBug = setting.limitBug[users[m.sender].rank];
        await fs.writeFileSync('./database/json/user.json', JSON.stringify(users, null, 2))
    }
    ...
    async function resetLimit() {
        ...
        let jumLm = Us.filter(v => v.limit <= 1 || v.limitBug < setting.limitBug[users[v.id].rank]).length
        if (jumLm > 1) {
            rmsg.limit.berhasil(conn, setting.group.komunitas, { jumlah: jumLm, waktu: moment.tz("Asia/Jakarta").format("DD MMMM YYYY") })
        } else {
            rmsg.limit.tidakjadi(conn, setting.group.komunitas)
        }
        ...
        Us.forEach(v => {
            ...
            if (v.limitBug) {
                let rLimit = setting.limitBug[users[v.id].rank]
                if (v.limitBug < rLimit) v.limitBug = rLimit
            }
            if (v.limitBug === 0) {
                v.limitBug = setting.limitBug[users[v.id].rank]
            }
        })
        await fs.writeFileSync('./database/json/user.json', JSON.stringify(users, null, 2))
    }
    ...
    cron.schedule("00 00 * * *", async() => {
        let today = moment.tz("Asia/Jakarta").format("YYYYMMDD")
        if (today >= db.data.reset) {
            await resetLimit()
            db.data.reset = moment.tz("Asia/Jakarta").add(1, "day").format("YYYYMMDD")
            await db.write()
        }
    ...
    else if (cmd.limit && users[m.sender].limit <= 0) return m.reply(`Penggunaan harian anda telah habis, Perintah ini setidaknya membutuhkan ${Number(cmd.limit)} Limit\n\nLimit direset setiap pukul 00.00 WIB, gunakan kembali setelah limit direset\n\nAtau kamu bisa topup untuk membeli limit tambahan dengan menggunakan perintah \`#topup\` bisa juga dengan upgrade rank untuk mendapatkan lebih banyak limit \`#upgrade platinum 30d\``)
    else if (cmd.limit && users[m.sender].limit < Number(cmd.limit)) return m.reply(`Limit anda kurang, Perintah ini setidaknya membutuhkan ${Number(cmd.limit)} Limit\n\nLimit direset setiap pukul 00.00 WIB, gunakan kembali setelah limit direset\n\nAtau kamu bisa topup untuk membeli limit tambahan dengan menggunakan perintah \`#topup\` bisa juga dengan upgrade rank untuk mendapatkan lebih banyak limit \`#upgrade platinum 30d\``)
    ...
    try {
        ...
        .catch(async(e) => {
            ...
            await errormes(conn, { e, ctx: m, command })
        });
        if (cmd.limit && users[m.sender].limit >= Number(cmd.limit)) {
            users[m.sender].limit -= Number(cmd.limit)
            await fs.writeFileSync('./database/json/user.json', JSON.stringify(users, null, 2))
        }
    } catch (e) {
        addhit(cmd.name, false);
