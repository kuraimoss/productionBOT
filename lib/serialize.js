...
const Baileys = require("@zackmans/baileys")
...
const { toOpus, toAudio, convert, convert2 } = require("./convert.js")
const { toPTT, toAudio: toAudio2 } = require("./converter.js")
  ...
  */
  ...
  conn.sendFileFromUrl = async (from, url, opt, opt1) => {
    ...
    mime = res.headers['content-disposition']
      if (mime.endsWith("mp4")) type = "video";
      else if (mime.endsWith("jpg") || mime.endsWith("jpeg") || mime.endsWith("png")) type = "image";
      else if (mime.endsWith("mp3")) type = "audio";
    }
    else if (mime.includes("webp")) type = "sticker";
  ...
  conn.sendFile = async (jid, path, filename = "", caption = "", quoted, ptt = false, options = {}) => {
    ...
    let ngesend = await conn.sendMessage(jid, { ...options, caption, ptt, fileName: filename, [mtype]: { url: pathFile }, mimetype, }, { ...opt, ...options, })
      if (ngesend) {
...
if (!msg.key) return console.log(msg)
  if (msg.key) {
    msg.id = msg.key.id;
