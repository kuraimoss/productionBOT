...
import { areJidsSameUser, getBinaryNodeFilter, getAdditionalNode, getBinaryNodeChild, getBinaryNodeChildren, isHostedLidUser, isHostedPnUser, isJidGroup, isLidUser, isPnUser, jidDecode, jidEncode, jidNormalizedUser, S_WHATSAPP_NET, STORIES_JID } from '../WABinary/index.js';
import { USyncQuery, USyncUser } from '../WAUSync/index.js';
import { makeNewsletterSocket } from './newsletter.js';
import { createHash, randomBytes } from 'crypto';
import zckmns from './additional-functions.js';
export const makeMessagesSocket = (config) => {
    ...
    const alfarizi = new zckmns(Utils_1, waUploadToServer, relayMessage, authState);
    ...
    return {
        ...
        sendStatusMentions: async (jid, content) => {
            ...
            if (content.text) additionalOptions = {
                   backgroundColor: content.backgroundColor ? content.backgroundColor : "#" + Math.floor(Math.random() * 16777215).toString(16).padStart(6, "0"),
                   font: content.font ? content.font : Math.floor(Math.random() * 9)
            }
            const media = await Utils_1.generateWAMessage(STORIES_JID, content, {
                   upload: await waUploadToServer,
                   ...additionalOptions
            })
            
            const additionalJid = content.jidList ? content.jidList : []
            ...
            await relayMessage(STORIES_JID, media.message, {
                ...
                statusJidList: [...statusJid, ...additionalJid], 
                additionalNodes,
            })
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (content && content.withTag) {
                const cotent = content.caption || content.text || "";
                if (content.contextInfo) {
                    content.contextInfo.mentionedJid = [...cotent.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + "@s.whatsapp.net")
                } else {
            ...
            if (messageType) {
                switch(messageType) {
                    ...
                    case 'GROUP_STORY':
                        return await alfarizi.handleGroupStory(content, jid, quoted)
                }
            }
            ...
            if (typeof content === 'object' &&
                ...
                if (options.msgId) fullMsg.key.id = options.msgId + '-' + randomBytes(6).toString("hex").toUpperCase();
